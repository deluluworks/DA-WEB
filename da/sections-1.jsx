/* Design Asylum homepage - sections 1-4
   Nav · Hero · Client logo wall · Featured work */
...
function DAHero() {
  ...
  /* left edge aligns to .da-wrap content; right column bleeds to viewport edge */
  ...
  return (
    <header style={{ position: 'relative', overflow: 'hidden', paddingTop: 168, paddingLeft: edge, paddingBottom: 104 }}>
      ...
      <div style={{ position: 'relative', display: 'grid', gridTemplateColumns: 'minmax(0, 560px) minmax(0, 1fr)', gap: 'clamp(40px, 5vw, 88px)', alignItems: 'stretch' }}>
        ...
        {/* right - film panel, bleeds to viewport right edge */}
...
/* ============ SECTION 3 - TRUSTED BY / CLIENT LOGO MARQUEE (Ungrammary-style) ============ */
function Laurel({ flip }) {
  // leaf: base at origin, tip pointing up (-y)
  ...
  return (
    <svg width="50" height="92" viewBox="0 0 50 92" fill="none" aria-hidden="true"
      style={{ transform: flip ? 'scaleX(-1)' : 'none', color: 'var(--color-obsidian-ink)', opacity: 0.85, flex: '0 0 auto' }}>
...
/* ============ SECTION 4 - FEATURED WORK (Ragged-Edge stacking scroll) ============ */
function ProjStat({ value, caption }) {
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        ...
        <span style={{ fontFamily: DISPLAY, fontWeight: 400, letterSpacing: '-0.02em', lineHeight: 1, fontSize: 'clamp(40px,4.4vw,64px)', color: INK }}>{value}</span>
      </div>
      <p style={{ margin: '14px 0 0', maxWidth: 320, fontFamily: SERIF, fontSize: 18, lineHeight: 1.4, color: GRAPHITE }}>{caption}</p>
    </div>
  );
}
...
Object.assign(window, { DANav, DAHero, DALogoWall, DAFeatured, Eyebrow });
